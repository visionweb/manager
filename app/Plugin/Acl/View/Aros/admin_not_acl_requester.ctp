...
<div class="error">
	...
	if(isset($group_is_not_requester))
	{
		echo '<p class="warning">';
		echo sprintf(__d('acl', 'The <em>%s</em> model is not configured to act as an <em>ACL requester</em>'), Configure :: read('acl.aro.group.model')) . '</p>';
		echo '</p>';
	}
	...
	echo '<p>';
	echo sprintf(__d('acl', 'In a classical ACL configuration, the models that represent the users and the groups must act as <em>ACL requesters</em> (see %s).'), 
