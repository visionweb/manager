...
class AclManagerComponent extends Component
{
    ...
    public function initialize(Controller $controller) //&$controller
	{
...
//		}
	...
	public function save_permission($aro_nodes, $aco_path, $permission_type)
	{
	    if(isset($aro_nodes[0]))
	    {
	        ...
	        $pk_name = 'id';
	        if($aro_nodes[0]['Aro']['model'] == Configure :: read('acl.aro.group.model'))
	        {
	            $pk_name = $this->controller->_get_group_primary_key_name();
	        }
    	    ...
    	    elseif(isset($specific_permission_right))
    	    {
            	...
            	if(!empty($aco_node))
            	{
        	        ...
        	        if(!empty($specific_permission) )//$specific_permission !== false && isset($specific_permission['Permission']))
        	        {
	...
	{
	    $pk_name = 'id';
        if($aro_node['Aro']['model'] == Configure :: read('acl.aro.group.model'))
        {
            $pk_name = $this->controller->_get_group_primary_key_name();
        }
    	...
    	if(!empty($aco_node))
    	{
    	    ...
    	    if(!empty($specific_permission))//$specific_permission !== false)
    	    {
    	        /*
	...
	private function get_first_parent_permission_right($aro_node, $aco_path)
	{
    	...
    	while(strpos($aco_path, '/') !== false && !isset($parent_permission_right))
        {
	    	...
	    	if(!empty($parent_aco_node))
	    	{
	    	    ...
	    	    if(!empty($parent_permission))//if($parent_permission !== false)
	    	    {
	    	        /*
